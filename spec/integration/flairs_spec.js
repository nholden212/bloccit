...
describe("routes : flairs", () => {
  beforeEach((done) => {
    ...
    sequelize.sync({force: true}).then((res) => {
      Topic.create({
        title: "Always Sunny",
        description: "A hilarious and wonderful show"
      })
      .then((topic) => {
        this.topic = topic;
        Post.create({
          title: "The Nightman Cometh",
          body: "You gotta pay the troll toll",
  ...
  describe("POST /topics/:topicId/posts/:postId/flairs/create", () => {
    it("should create a new flair and redirect", (done) => {
      const options = {
        ...
        form: {
          name: "Thundergun express",
          color: "Brown"
        }
      };
      request.post(options,
        (err, res, body) => {
          Flair.findOne({where: {name: "Thundergun express"}})
          .then((flair) => {
            expect(flair).not.toBeNull();
            expect(flair.name).toBe("Thundergun express");
  ...
  describe("GET /topics/:topicId/posts/:postId/flairs/:id", () => {
    it("should render a view with the selected flair", (done) => {
      request.get(`${base}/${this.topic.id}/posts/${this.post.id}/flairs/${this.flair.id}`, (err, res, body) => {
        expect(err).toBeNull();
        expect(body).toContain("Milk steak");
        done();
      });
    });
  });
  ...
  describe("POST /topics/:topicId/posts/:postId/flairs/:id/update", () => {
     ...
     it("should update the flair with the given values", (done) => {
         ...
         request.post(options,
           ...
           .then((flair) => {
             expect(flair.name).toBe("Milk steak");
             done();
           });
         });
     });
